...
class Evaluate:
    ...
    def do_iou(self, pred_y, true_y, tp, fp, fn, all, t_unassigned, errors):
        ...
        tp = tf.tensor_scatter_nd_add(tp, p_classes_indices, updates)
        updates = tf.cast(tf.math.logical_not(res), dtype=tf.int32)
        fp = tf.tensor_scatter_nd_add(fp, p_classes_indices, updates)
        ...
        indices = tf.expand_dims(t_classes_indices, axis=-1)
        all = tf.tensor_scatter_nd_add(all, indices, updates)
        return tp, fp, fn, all, errors
    ...
    def calc_ap(self, gt_classes, tfrecords_dir, evaluate_iou_threshold, image_size, classes_name_file, model_config_file, input_weights_path, anchors_table,
        ...
        all = tf.convert_to_tensor([0] * nclasses)
        ...
        for idx, (pred_y, true_y) in enumerate(data):
            ...
            tp, fp, fn, all, errors = tf.cond(tf.shape(true_y)[0] != 0,
                                                 true_fn=lambda: self.do_iou(pred_y, true_y, tp, fp, fn, all, t_unassigned, errors),
                                                  false_fn=lambda: (tp, fp,
                                                  tf.tensor_scatter_nd_add(fn,
                                                                           tf.expand_dims(tf.cast(true_y[..., 4], tf.int32), axis=-1),
                                                                           tf.fill(tf.shape(true_y)[0], 1)),

                                                  tf.tensor_scatter_nd_add(all,
                                                                           tf.expand_dims(
                                                                               tf.cast(true_y[..., 4], tf.int32),
                                                                               axis=-1),
            ...
            print(tp.numpy(), fp.numpy(), fn.numpy(), all.numpy(), errors.numpy())

        tn = all - tp -fp - fn

        accuracy = tf.cast(tp+tn, tf.float32)/(tf.cast(all, tf.float32) + 1e-10)
